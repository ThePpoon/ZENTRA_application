...
def main():
    ...
    ap.add_argument("--fast",   action="store_true",
                    help="CPU mode: epochs น้อยลง, aug น้อยลง (เทรนเร็วขึ้น ~3x)")
    ...
    # LSTM specific
    ap.add_argument("--lstm-epochs", type=int, default=1000)
    ap.add_argument("--lstm-batch",  type=int, default=1000)
    ap.add_argument("--lstm-lr",     type=float, default=1e-3)
    ap.add_argument("--lstm-aug-x",  type=int, default=1)
    args = ap.parse_args()

    # --fast: CPU-friendly settings
    import torch as _torch
    _has_gpu = _torch.cuda.is_available()
    if args.fast or not _has_gpu:
        if not _has_gpu:
            print("  [Auto] ไม่มี GPU — ปรับ settings ให้เหมาะกับ CPU")
        PPE_CFG["epochs"]          = min(PPE_CFG["epochs"],  20)
        PPE_CFG["aug_multiplier"]  = 1
        PPE_CFG["batch"]           = 8
        PPE_CFG["workers"]         = 0
        POSE_CFG["epochs"]         = min(POSE_CFG["epochs"], 20)
        POSE_CFG["aug_multiplier"] = 1
        POSE_CFG["batch"]          = 8
        POSE_CFG["workers"]        = 0
        if not hasattr(args, "lstm_epochs") or args.lstm_epochs == 20:
            args.lstm_epochs = 20
        args.lstm_aug_x = 1
